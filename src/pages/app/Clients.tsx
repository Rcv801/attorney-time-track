...
export default function Clients() {
  ...
  const { data: clients } = useQuery({
    queryKey: ["clients"],
    queryFn: async () => {
      const { data, error } = await supabase.from("clients").select("id,name,archived,hourly_rate").order("name");
  ...
  const add = async () => {
    ...
    await supabase.from("clients").insert({ name, user_id: user.id, hourly_rate });
    setName("");
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid sm:grid-cols-3 gap-2 items-center">
