...
create policy if not exists "Clients are viewable by owner"
on public.clients
for select
using (user_id = auth.uid());

create policy if not exists "Clients are insertable by owner"
on public.clients
for insert
with check (user_id = auth.uid());

create policy if not exists "Clients are updatable by owner"
on public.clients
for update
using (user_id = auth.uid())
with check (user_id = auth.uid());

create policy if not exists "Clients are deletable by owner"
on public.clients
for delete
using (user_id = auth.uid());
...
create policy if not exists "Entries are viewable by owner"
on public.entries
for select
using (user_id = auth.uid());

create policy if not exists "Entries are insertable by owner"
on public.entries
for insert
with check (user_id = auth.uid());

create policy if not exists "Entries are updatable by owner"
on public.entries
for update
using (user_id = auth.uid())
with check (user_id = auth.uid());

create policy if not exists "Entries are deletable by owner"
on public.entries
for delete
using (user_id = auth.uid());
